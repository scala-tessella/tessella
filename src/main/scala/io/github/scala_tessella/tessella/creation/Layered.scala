...
trait Layered extends Reticulate:
  ...
  private def triHexBase(l: Int, h: Int = 0)(f: (Int, Int) => Boolean): (List[Edge], Node) =
    ...
    val edges: List[Edge] =
      pattern_333333((l + 1) * 2, h + 1).toOption.get.graphEdges.withoutNodes(topRight :: bottomLeft :: emptyNodes.toList).compact
  ...
  private val fs: Map[Int, Int => (List[Edge], Node)] = Map(
    3 -> { l => (pattern_333333(l * 2, 1).toOption.get.graphEdges, Node(l + 2)) },
    4 -> { l => (pattern_4444(l, 1).toOption.get.graphEdges, Node(l + 2)) },
